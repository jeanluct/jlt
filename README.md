...
There isn't any.  In fact this library should really not be used by anyone, and I've mostly uploaded it so I can easily include it in other projects.  If you feel brave, many functions have comments documenting their use, and there are examples in the [examples][2] folder.

### testing

A comprehensive test suite using Catch2 is available in the `tests/` directory. It includes:
- 7 core test suites (446+ assertions) for the header-only library
- 2 optional LAPACK-dependent test suites (60 assertions) for eigensystem and SVD

```bash
cd tests && mkdir -p build && cd build && cmake .. && make && ctest
```

See `tests/README.md` for detailed testing documentation.

Here is a list of some of the more useful C++ functions and classes, in the folder [jlt][3].  They can be included as `#include "jlt/<filename>.hpp"`, so the folder containing `jlt` must be in the include files search path.  The programs in [examples][2] are readily compiled with the [SCONS][4] build tool, but unless linking to external libraries is required `g++ -O <prog>.cpp -o <prog> -I..` is usually sufficient.
...
* `jlt::tictoc` works a bit like Matlab's `tic` and `toc` to time programs.  It uses `boost/timer/timer.hpp`.  See the example program `tictoc_test.cpp`.
...
[2]: https://github.com/jeanluct/jlt/tree/master/examples
